/*!
BINE v7.9 - CUSTOM BINE-OPTIMIZED HASH
=======================================

Instead of ChaCha20 (which has differential bias issues),
create a CUSTOM hash primitive designed specifically for BINE.

Design Goals:
1. Differential bias < 0.01 (PRIMARY)
2. Avalanche ~50% (maintain what works)
3. Reasonable speed (>100 MB/s)
4. Simple, auditable design

Custom BineHash Design:
- Multi-round Feistel-like structure
- Heavy S-box usage (proven non-linear)
- Position-dependent mixing
- Multiple independent mix paths
- Designed from scratch for our exact needs
*/
...
impl BineHasherV79 {
    ...
    pub fn hash(&mut self, data: &[u8], salt: &[u8]) -> Vec<u8> {
        ...
        // v7.9: Use CUSTOM BINE hash instead of ChaCha20-based mini_hash
        let deterministic_seed = bine_hash(&seed_data, self.state_size);
        let mut state = bine_hash(&salted, self.state_size);
    ...
    fn ostrich_diversify(&self, state: &[u8]) -> Vec<u8> {
        ...
        bine_hash(&variants, self.state_size)
    }
    ...
    fn final_nonlinear_output(&self, state: &[u8]) -> Vec<u8> {
        ...
        let checksum = bine_hash(&checksum_data, 16);
    ...
    fn derive_round_key(&self, seed: &[u8], round: usize) -> Vec<u8> {
        ...
        bine_hash(&round_data, self.state_size)
    }
}

// v7.9: CUSTOM BINE HASH - OPTIMIZED for speed + differential resistance
// Design: Block-based (64-byte blocks) with 4 total rounds
fn bine_hash(data: &[u8], output_size: usize) -> Vec<u8> {
    // Initialize state with constants
    let mut state = vec![0u8; output_size.max(64)];
    let magic = [0x42, 0x49, 0x4e, 0x45]; // "BINE"
    for i in 0..state.len() {
        state[i] = magic[i % 4] ^ ((i * 251) % 256) as u8;
    }
    ...
    // Process data in 64-byte blocks (MUCH faster than 4-byte)
    for block in data.chunks(64) {
        // Mix block into state
        for (i, &byte) in block.iter().enumerate() {
            let si = i % state.len();
            let prev = state[if si > 0 { si - 1 } else { state.len() - 1 }];

            // Double S-box for non-linearity
            let temp1 = SBOX[(state[si] ^ byte) as usize];
            let temp2 = SBOX[(temp1 ^ prev) as usize];

            state[si] = temp2.wrapping_mul(251);
        }

        // Single mixing round after each block
        for i in 0..state.len() {
            let prev = state[if i > 0 { i - 1 } else { state.len() - 1 }];
            let next = state[if i < state.len() - 1 { i + 1 } else { 0 }];
            state[i] = SBOX[(state[i] ^ prev ^ next) as usize];
        }
    }

    // 4 final mixing rounds for good diffusion
    for round in 0..4 {
        for i in 0..state.len() {
            let prev = state[if i > 0 { i - 1 } else { state.len() - 1 }];
            let next = state[if i < state.len() - 1 { i + 1 } else { 0 }];
            let far = state[(i + state.len() / 2) % state.len()];

            let temp1 = SBOX[state[i] as usize];
            let temp2 = SBOX[(temp1 ^ prev) as usize];
            let temp3 = SBOX[(temp2 ^ next) as usize];
            let temp4 = SBOX[(temp3 ^ far) as usize];

            state[i] = temp4.wrapping_mul(179).rotate_left((round % 8) as u32);
        }
    }

    // Final S-box and truncate

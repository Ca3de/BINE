/*!
BINE v7.6 - AGGRESSIVE Differential Cryptanalysis Fix
======================================================

v7.5 Results:
✓ Avalanche: 50.09% (PERFECT!)
✗ Differential: 0.5000 (WORST POSSIBLE!)

v7.6 Strategy - BREAK ALL LINEAR PATTERNS:
1. Multiple S-box layers (3x per round)
2. Aggressive cross-mixing (every byte affects distant bytes)
3. Non-linear feedback loops
4. Key-dependent rotations and XORs
5. Differential-specific diffusion layer

Target: Differential bias < 0.01 (ideally < 0.005)
*/
...
impl NeuralLearning {
    ...
    pub fn extract_features(&self, data: &[u8]) -> Vec<f64> {
        ...
        // Features 56-63: Sequential patterns
...
impl BineHasherV76 {
    ...
    pub fn hash(&mut self, data: &[u8], salt: &[u8]) -> Vec<u8> {
        ...
        for round in 0..total_rounds {
            // v7.6: TRIPLE non-linear layer per round
            state = self.anti_differential_transform(&state, round, &deterministic_seed);
    ...
    // v7.6: AGGRESSIVE anti-differential transform
    // Strategy: 3x S-box layers + aggressive cross-mixing + feedback
    fn anti_differential_transform(&self, state: &[u8], round: usize, seed: &[u8]) -> Vec<u8> {
        let round_key = self.derive_round_key(seed, round);
        let len = state.len();

        // Layer 1: S-box with key mixing
        let mut layer1 = Vec::with_capacity(len);
        for i in 0..len {
            let s = state[i];
            let k = round_key[i % round_key.len()];
            let mixed = s.wrapping_add(k);
            layer1.push(SBOX[mixed as usize]);
        }

        // Layer 2: Cross-mixing (each byte affects 4 distant positions)
        let mut layer2 = vec![0u8; len];
        for i in 0..len {
            let pos1 = i;
            let pos2 = (i + len / 4) % len;
            let pos3 = (i + len / 2) % len;
            let pos4 = (i + 3 * len / 4) % len;

            // Aggressive non-linear mixing
            let v1 = layer1[pos1];
            let v2 = layer1[pos2].rotate_left(3);
            let v3 = layer1[pos3].rotate_right(5);
            let v4 = layer1[pos4].rotate_left(7);

            let mixed = v1 ^ v2 ^ v3 ^ v4;
            let sboxed = SBOX[mixed as usize];
            layer2[i] = sboxed.wrapping_mul(251); // Non-linear multiply
        }

        // Layer 3: Feedback with double S-box
        let mut layer3 = Vec::with_capacity(len);
        for i in 0..len {
            let prev = if i > 0 { layer2[i - 1] } else { layer2[len - 1] };
            let curr = layer2[i];
            let next = if i < len - 1 { layer2[i + 1] } else { layer2[0] };

            // Double S-box for maximum non-linearity
            let temp1 = SBOX[(curr ^ prev) as usize];
            let temp2 = SBOX[(temp1 ^ next) as usize];

            // Key-dependent rotation
            let rot_amount = (round_key[i % round_key.len()] % 8) as u32;
            layer3.push(temp2.rotate_left(rot_amount));
        }

        // Layer 4: Final diffusion pass to spread changes
        let mut output = vec![0u8; len];
        for i in 0..len {
            let mut acc = layer3[i];

            // Mix with 8 different positions
            for j in 1..9 {
                let pos = (i + j * len / 8) % len;
                acc ^= layer3[pos].rotate_left(j as u32);
            }

            output[i] = SBOX[acc as usize];
        }

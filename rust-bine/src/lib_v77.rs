/*!
BINE v7.7 - ROOT CAUSE FIX for Differential
============================================

v7.6 Analysis:
The max bias of 0.5000 means some bits are COMPLETELY predictable.
Root cause: The output checksum/padding bytes weren't going through
proper non-linear transformations!

v7.7 Fix:
1. Apply S-box to EVERY output byte
2. Ensure final output goes through complete non-linear pass
3. Add final mixing pass that treats all bytes equally
4. No untransformed bytes in the output
...
impl BineHasherV77 {
    ...
    pub fn hash(&mut self, data: &[u8], salt: &[u8]) -> Vec<u8> {
        ...
        // v7.7: Ensure final output has ZERO untransformed bytes
    ...
    // v7.7: ROOT CAUSE FIX - ensure EVERY output byte is non-linear
    fn final_nonlinear_output(&self, state: &[u8]) -> Vec<u8> {
        ...
        // Combine state + checksum
        let mut combined = state.to_vec();
        combined.extend_from_slice(&checksum);

        // v7.7 FIX: Apply S-box to EVERY byte
        for byte in combined.iter_mut() {
            *byte = SBOX[*byte as usize];
        }

        // Final mixing pass - each byte affects neighbors
        ...
        for i in 0..len {
            ...
            // Triple S-box for maximum non-linearity
